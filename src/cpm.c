...
static void cpm_env_get_runtime (void) {
...
static unsigned int cpm_env_get_port (void) {
...
static void cpm_env_get_cerver_receive_buffer_size (void) {
...
static void cpm_env_get_cerver_th_threads (void) {
...
static void cpm_env_get_cerver_connection_queue (void) {
...
static unsigned int cpm_init_env (void) {

	unsigned int errors = 0;

	cpm_env_get_runtime ();

	errors |= cpm_env_get_port ();

	cpm_env_get_cerver_receive_buffer_size ();

	cpm_env_get_cerver_th_threads ();

	cpm_env_get_cerver_connection_queue ();
...
unsigned int cpm_init (void) {

	unsigned int errors = 0;

	if (!cpm_init_env ()) {
		errors |= cpm_service_init ();
	}
